...
export default function page(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/dashboard/tickets`,{
                method: "GET",
                headers: {
                ...
                <div className="rounded-2xl bg-[#151F30] w-full h-full p-5 overflow-x-auto">
                    <table className="table-auto text-start border-separate border-spacing-y-3 overflow-scroll w-full">
                        ...
                        <tr className="bg-[#1E293B]">
                            <th className="text-start p-3 text-xs text-[#e0e0e0] rounded-r-2xl">userId</th>
                            <th className="text-start p-3 text-xs text-[#e0e0e0]l">Title</th>
                        ...
                        {tickets.map((ticket) => (
                            <tr key={ticket.id} className="text-gray-300 hover:bg-[#1E293B] rounded-lg">
                                <td className="p-3 text-sm">{ticket.userId}</td>
